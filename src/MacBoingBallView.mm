...
static os_log_t getLog() {
    ...
    dispatch_once(&onceToken, ^{
        log = os_log_create("com.adamb3ll.BoingBallSaver", "screensaver");
    });
    return log;
}
...
- (instancetype)initWithFrame:(NSRect)frame isPreview:(BOOL)isPreview {
    ...
    if (self) {
        // CRITICAL: Clean up any existing resources first (in case view is reused)
        // This prevents resource leaks if initWithFrame is called multiple times
        ...
        [self setAnimationTimeInterval:1.0/120.0];  // 120 FPS
...
- (void)dealloc {
    ...
    // CRITICAL: Clean up OpenGL resources BEFORE releasing the context
    // The renderer needs a valid OpenGL context to delete textures, etc.
    if (_renderer && _glContext) {
        [_glContext makeCurrentContext];
        _renderer->Cleanup();  // Explicitly clean up OpenGL resources while context is valid
        
        // Flush any pending OpenGL commands before releasing context
        glFlush();
        
        // Clear the current context before releasing it
        [NSOpenGLContext clearCurrentContext];
    }
    
    // Now safe to delete the renderer (destructor won't try to delete OpenGL resources)
    if (_renderer) {
        delete _renderer;
        _renderer = nullptr;
    }
...
- (void)setupOpenGL {
    // Clean up any existing OpenGL resources first (defensive)
    if (_glContext) {
...
- (void)initializeModules {
    // Clean up any existing modules first (defensive)
    if (_renderer && _glContext) {
        [_glContext makeCurrentContext];
        _renderer->Cleanup();
        [NSOpenGLContext clearCurrentContext];
    ...
    _renderConfig->ballLightingEnabled = _config->enableBallLighting;
    _renderConfig->showFPS = _config->showFPS;
...
- (void)cleanupAllResources {
    ...
    // Clean up OpenGL resources BEFORE releasing context
    if (_renderer && _glContext) {
        [_glContext makeCurrentContext];
        _renderer->Cleanup();
        glFlush();
...
- (void)cleanupOpenGL {
    if (_glContext) {
        // Ensure context is not current before releasing
        if ([NSOpenGLContext currentContext] == _glContext) {
            [NSOpenGLContext clearCurrentContext];
        }
        [_glContext clearDrawable];
        [_glContext release];
        _glContext = nil;
        _cachedCGLContext = nil;  // Clear cached CGL context reference
    }
    if (_glPixelFormat) {
        [_glPixelFormat release];
        _glPixelFormat = nil;
...
- (void)viewDidMoveToWindow {
    ...
    if (self.window && _glContext) {
        ...
        // IMPORTANT: Always invalidate existing timer first to prevent multiple timers
        if (_fullscreenTimer) {
            [_fullscreenTimer invalidate];
            _fullscreenTimer = nil;
        }
        
        if (![self isPreview] && _physics && _platform) {
            _prevTime = _platform->GetHighResolutionTime();
            _isAnimating = YES;  // Mark animation as active
            
            // Use 120 FPS timer for better performance
            // Use common modes to ensure timer fires even during UI events
            _fullscreenTimer = [NSTimer timerWithTimeInterval:1.0/120.0
                                                       target:self
...
- (void)renderFrame {
    ...
    [_glContext makeCurrentContext];
    // NOTE: Do NOT call update() here - it's expensive and only needed when view moves to new window
    // update() is already called in viewDidMoveToWindow and setFrameSize
    
    // Get current bounds - always use bounds (view-relative) not frame (window-relative)
    NSRect bounds = [self bounds];
    
    // Only update viewport if size changed (avoid recalculating projection matrix every frame)
    if (bounds.size.width != _cachedBounds.width || bounds.size.height != _cachedBounds.height) {
        _cachedBounds = bounds.size;
        float wallX, wallZ, floorY;
        _renderer->SetViewport((int)bounds.size.width, (int)bounds.size.height, wallX, wallZ, floorY);
        if (_physics) {
    ...
    // Calculate delta time for FPS display
    double currentTime = _platform->GetHighResolutionTime();
    float dt = (float)(currentTime - _prevTime);
    if (dt <= 0.0f || dt > 0.1f) dt = 1.0f / 120.0f;  // Default to 120 FPS if invalid
    _prevTime = currentTime;
    
    // Render
    _renderer->RenderFrame(*_physics, *_renderConfig, dt);
    
    // flushBuffer handles the swap - no need for glFlush() which forces immediate execution
    // and can hurt performance. The swap buffer mechanism handles synchronization.
    [_glContext flushBuffer];
}

- (NSTimeInterval)animationTimeInterval {
    return 1.0/120.0;  // 120 FPS for better performance
}
...
- (void)timerFired:(NSTimer*)timer {
    // For fullscreen mode, render directly for better performance
    // This avoids the overhead of setNeedsDisplay -> drawRect -> renderFrame
    if (![self isPreview]) {
        [self animateAndRenderFrame];
    } else {
...
- (void)animateOneFrame {
    ...
    double currentTime = _platform->GetHighResolutionTime();
    float dt = (float)(currentTime - _prevTime);
    if (dt <= 0.0f || dt > 0.1f) dt = 1.0f / 60.0f;  // Cap delta time
    _prevTime = currentTime;
...
// Optimized animation and rendering for fullscreen mode
// Combines physics update and rendering in one call to avoid setNeedsDisplay overhead
- (void)animateAndRenderFrame {
    // Check if animation is still active - if not, don't do anything
    if (!_isAnimating) {
        return;
    }
    ...
    // Play sounds for new collisions
    BOOL shouldPlaySound = _isAnimating && _config->enableSound && ![self isPreview] && 
                           [window isVisible];
    ...
    // Render directly - no need for setNeedsDisplay overhead
    [self renderFrame];
}
...
- (NSWindow*)configureSheet {
    ...
    if (_configSheet) {
        ...
        [_floorShadowCheckbox setState:_config->enableFloorShadow ? NSControlStateValueOn : NSControlStateValueOff];
        [_wallShadowCheckbox setState:_config->enableWallShadow ? NSControlStateValueOn : NSControlStateValueOff];
        [_gridCheckbox setState:_config->enableGrid ? NSControlStateValueOn : NSControlStateValueOff];
        [_soundCheckbox setState:_config->enableSound ? NSControlStateValueOn : NSControlStateValueOff];
        [_geometryCheckbox setState:_config->smoothGeometry ? NSControlStateValueOff : NSControlStateValueOn];
        [_ballLightingCheckbox setState:_config->enableBallLighting ? NSControlStateValueOn : NSControlStateValueOff];
        [_fpsCheckbox setState:_config->showFPS ? NSControlStateValueOn : NSControlStateValueOff];
...
- (void)createConfigSheetProgrammatically {
    // Create window - wider to accommodate Cancel button, taller for FPS checkbox
    _configSheet = [[NSWindow alloc] initWithContentRect:NSMakeRect(0, 0, 420, 340)
    ...
    // Ball lighting checkbox
    _ballLightingCheckbox = [[NSButton alloc] initWithFrame:NSMakeRect(20, y, 300, 20)];
    [_ballLightingCheckbox setButtonType:NSButtonTypeSwitch];
    [_ballLightingCheckbox setTitle:@"Enable ball lighting"];
    [contentView addSubview:_ballLightingCheckbox];
    y -= 30;
    
    // FPS checkbox
    _fpsCheckbox = [[NSButton alloc] initWithFrame:NSMakeRect(20, y, 300, 20)];
    [_fpsCheckbox setButtonType:NSButtonTypeSwitch];
    [_fpsCheckbox setTitle:@"Show FPS counter"];
    ...
    if ([sender isKindOfClass:[NSButton class]]) {
        ...
        if ([[button title] isEqualToString:@"OK"]) {
            ...
            if (!_floorShadowCheckbox || !_wallShadowCheckbox || !_gridCheckbox || 
                !_soundCheckbox || !_geometryCheckbox || !_ballLightingCheckbox || 
                !_fpsCheckbox || !_colorWell) {
            ...
            // Save settings
            _config->enableFloorShadow = ([_floorShadowCheckbox state] == NSControlStateValueOn);
            _config->enableWallShadow = ([_wallShadowCheckbox state] == NSControlStateValueOn);
            _config->enableGrid = ([_gridCheckbox state] == NSControlStateValueOn);
            _config->enableSound = ([_soundCheckbox state] == NSControlStateValueOn);
            _config->smoothGeometry = ([_geometryCheckbox state] == NSControlStateValueOff);
            _config->enableBallLighting = ([_ballLightingCheckbox state] == NSControlStateValueOn);
            _config->showFPS = ([_fpsCheckbox state] == NSControlStateValueOn);
...
- (IBAction)restoreDefaults:(id)sender {
    ...
    [_floorShadowCheckbox setState:NSControlStateValueOn];
    [_wallShadowCheckbox setState:NSControlStateValueOn];
    [_gridCheckbox setState:NSControlStateValueOn];
    [_soundCheckbox setState:NSControlStateValueOn];
    [_geometryCheckbox setState:NSControlStateValueOff];
    [_ballLightingCheckbox setState:NSControlStateValueOn];
    [_fpsCheckbox setState:NSControlStateValueOff];
...
- (void)setShowFPS:(BOOL)showFPS {
    if (_config && _renderConfig) {
        _config->showFPS = showFPS;
